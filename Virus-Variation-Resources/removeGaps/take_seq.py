...
filename = input("please type the name of alignment sequence file to take sequence: ")
...
seqNum = int(input("number of sequences to take: "))
seqLength = int(input("length of each sequence: "))
...
for s in filename.split('.'):
	if (s != 'fa') and (s != 'fasta'):
		newFilename += s
newFilename += (str(seqNum) + '*' + str(seqLength) + '.fa')
file = open(newFilename, 'w+')
for seq in nameSeq[:seqNum]:
	file.write('>'+seq[0]+'\n')
	file.write(seq[1][:seqLength]+'\n')
file.close()